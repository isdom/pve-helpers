...
if [ "$#" -ne 1 ]; then
    echo "需要提供作为根目录的子卷的名称，usage: ./br-2.sh <subvol name>"
    exit -1
fi

sblid=$(btrfs sub list / | awk -v sbl=$1 '$9==sbl {print $2}')

echo "作为根目录启动的子卷ID为:$sblid"
if [[ $sblid =~ ^[0-9]+$ ]]; then
    echo "$1 为有效子卷名称"
else
...
echo "将下次启动的 btrfs 根文件系统设置为: "$1
echo "efi uuid=$eid rootfs uuid=$rid"
btrfs sub set $sblid /
btrfs sub get /
...
echo "change boot from "$1" success"
umount -lf $rfs_path
rm -r $rfs_path
